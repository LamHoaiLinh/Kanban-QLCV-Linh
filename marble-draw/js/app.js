...
function render(){if(!current){refs.main.innerHTML='<div class="md-card">Chưa có sự kiện.</div>';return}syncSoundButtons();const validation=validateParticipants(current.participants||[]);const eligible=eligibleParticipants(current).length;const mapped=!!current.mapping?.length;const locked=['list_locked','shuffled','ready','running','completed'].includes(current.status);const count=currentDigitCount();refs.main.innerHTML=`
  <div class="md-grid md-dashboard">
    ...
    <section class="md-card half">
      ...
      <p>1) Khóa danh sách → 2) Bấm xáo bao nhiêu lần tùy ý → 3) Khóa kết quả xáo để tạo mapping → 4) Bắt đầu đua.</p>
    ...
    <section class="md-card">
      <div class="view-head compact"><div><h3>Người tham dự</h3><p>Nạp từ Excel/CSV hoặc dán trực tiếp từ Excel. Tất cả xử lý ngay trên thiết bị.</p></div><div class="view-actions"><a class="md-secondary-btn" href="samples/Danh_sach_nguoi_tham_du_mau.xlsx" download>Tải file mẫu</a>${locked?'<button id="unlockListBtn" class="md-danger-btn">Mở khóa danh sách</button>':'<button id="lockListBtn" class="md-primary-btn">Khóa danh sách</button>'}<button id="validateBtn" class="md-secondary-btn">Kiểm tra dữ liệu</button></div></div>
      <div class="md-grid">
    ...
    <section class="md-card half">
      <div class="view-head compact"><div><h3>Xáo danh sách & gán mã</h3><p>Bạn có thể bấm xáo nhiều lần để trình diễn, sau đó mới khóa kết quả xáo.</p></div><div class="view-actions"><button id="trialShuffleBtn" class="md-secondary-btn">Xáo thử</button><button id="officialShuffleBtn" class="md-secondary-btn" ${!locked||mapped?'disabled':''}>Ghi nhận 1 lần xáo</button><button id="finalizeShuffleBtn" class="md-primary-btn" ${!locked||mapped?'disabled':''}>Khóa & tạo mã</button></div></div>
      <div class="md-toolbar"><span class="status-pill">Đã xáo chính thức: ${current.shuffleCount||0} lần</span>${mapped?'<span class="status-pill">Mapping đã sẵn sàng</span>':'<span class="status-pill warn">Chưa khóa mapping</span>'}</div>
      <div id="shuffleStage" class="shuffle-stage"><div id="shuffleCloud" class="shuffle-cloud">${shuffleNameHtml()}</div></div>
      <div class="inline-note" style="margin-top:10px">Sau khi bấm <b>Khóa & tạo mã</b>, danh sách mã số chính thức sẽ xuất hiện ngay bên phải.</div>
    </section>
    <section class="md-card half">
      <h3>Danh sách mã số chính thức</h3>
      ${mapped?`<div class="mapping-list">${current.mapping.map(m=>`<div class="mapping-item"><span class="mapping-code">${m.code}</span><span><strong>${esc(m.name)}</strong><small>${esc(m.participantId)} · ${esc(m.department||'')}</small></span></div>`).join('')}</div>`:'<div class="inline-note">Chưa có mapping. Hãy khóa danh sách và khóa kết quả xáo.</div>'}
    </section>

    <section class="md-card half">
      <div class="view-head compact"><div><h3>Bắt đầu đua</h3><p>Kết quả quay từ hàng cao nhất xuống hàng thấp nhất: ${digitLabels(count).join(' → ')}.</p></div><div class="view-actions"><button id="demoRaceBtn" class="md-secondary-btn">Chạy thử</button><button id="officialRaceBtn" class="md-primary-btn" ${mapped?'':'disabled'}>Chạy chính thức</button></div></div>
      ...
      <div class="inline-note">Khi bấm bắt đầu đua, hệ thống sẽ mở màn hình đường đua toàn màn hình. Nếu mã ra trống hoặc ra người không còn đủ điều kiện, hệ thống sẽ yêu cầu quay lại toàn bộ chuỗi.</div>
    </section>
    <section class="md-card half">
...
function bindMain(){
  ...
  if($('#lockListBtn'))$('#lockListBtn').onclick=lockList;
  if($('#unlockListBtn'))$('#unlockListBtn').onclick=unlockList;
  ...
  $('#finalizeShuffleBtn').onclick=finalizeCurrentShuffle;
...
async function finalizeCurrentShuffle(){if(!current.participantListHash)return showError(new Error('Hãy khóa danh sách trước.'));if(!current.shuffleCount){if(!confirm('Chưa ghi nhận lần xáo nào. Vẫn khóa mapping với số lần xáo bằng 0?'))return;}await finalizeShuffle(current);await persist('Đã khóa mapping và gán mã số.')}
