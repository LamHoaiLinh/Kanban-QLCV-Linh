...
const state = {
  ...
  imageFiles:[], imageCrop:{fileId:null,ratio:'free',selection:null}, imageComposer:{html:'',width:1080,format:'image/png',quality:92,bg:'#ffffff',normalizeText:true,name:'Noi_dung_dai'}, renameEntries:[], renameRootHandle:null, excelFiles:[],
...
// ===== IMAGE =====
function renderImageTool(){const tabs=[['convert','Chuyển đổi & resize'],['crop','Crop ảnh'],['compress','Nén ảnh'],['merge','Ghép ảnh'],['watermark','Đóng dấu'],['create','Tạo ảnh dài']];mainHost.innerHTML=`<section class="office-tool-panel active"><div class="office-tool-head"><div><h3>IMG</h3><p>Xử lý ảnh bằng Canvas API; ảnh và nội dung dán không được lưu vào Local Storage.</p></div></div>${subTabs('image',tabs)}<div id="imageSubHost"></div></section>`;bindSubtabs('image');const h=mainHost.querySelector('#imageSubHost');if(state.sub.image==='convert')renderImageConvert(h);if(state.sub.image==='crop')renderImageCrop(h);if(state.sub.image==='compress')renderImageCompress(h);if(state.sub.image==='merge')renderImageMerge(h);if(state.sub.image==='watermark')renderImageWatermark(h);if(state.sub.image==='create')renderLongImageComposer(h)}
...
function renderImageCrop(h){
  const current=state.imageFiles.find(x=>x.id===state.imageCrop.fileId)||state.imageFiles[0]||null;
  state.imageCrop.fileId=current?.id||null;
  const sourceOptions=state.imageFiles.map((it,i)=>`<option value="${it.id}" ${it.id===state.imageCrop.fileId?'selected':''}>${i+1}. ${escapeHtml(it.file.name)}</option>`).join('');
  const ratio=state.imageCrop.ratio||'free';
  imageLibrary(h,`<div class="office-card image-crop-card"><div class="office-card-title-row"><div><h4>Cắt ảnh theo tỷ lệ</h4><p class="office-card-note">Kéo trực tiếp trên ảnh để tạo vùng cắt. “Tự do” là mặc định; các tỷ lệ cố định sẽ khóa đúng khổ khi kéo.</p></div><span class="office-recommend-badge">Giữ độ phân giải gốc</span></div><div class="office-grid three"><label class="office-field"><span>Ảnh đang cắt</span><select id="imgCropSource" ${state.imageFiles.length?'':'disabled'}>${sourceOptions||'<option>Chưa có ảnh</option>'}</select></label><label class="office-field"><span>Tỷ lệ khung</span><select id="imgCropRatio"><option value="free">Tự do (mặc định)</option><option value="1:1">1:1 · Vuông</option><option value="4:5">4:5 · Chân dung</option><option value="5:4">5:4 · Ngang</option><option value="4:3">4:3 · Ngang</option><option value="3:4">3:4 · Dọc</option><option value="3:2">3:2 · Ảnh ngang</option><option value="2:3">2:3 · Ảnh dọc</option><option value="16:9">16:9 · Màn hình</option><option value="9:16">9:16 · Story/Reels</option><option value="210:297">A4 dọc</option><option value="297:210">A4 ngang</option></select></label><label class="office-field"><span>Định dạng đầu ra</span><select id="imgCropFormat"><option value="image/jpeg">JPG</option><option value="image/png">PNG</option><option value="image/webp">WebP</option></select></label><label class="office-field"><span>Chất lượng JPG/WebP</span><input id="imgCropQuality" type="number" min="10" max="100" value="${state.settings.imageCropQuality||100}"></label><label class="office-field"><span>Kích thước vùng cắt</span><input id="imgCropInfo" type="text" value="Chưa chọn ảnh" readonly></label></div><div id="imgCropPreviewWrap" class="image-crop-preview-wrap"><div class="office-empty">${current?'Đang tạo xem trước…':'Hãy chọn ảnh ở phía trên.'}</div></div><div class="office-toolbar"><button class="office-btn" id="imgCropReset" ${current?'':'disabled'}>Đặt lại khung</button><span class="office-card-note">Mẹo: nếu muốn đổi vùng, chỉ cần kéo lại trên ảnh.</span><span class="spacer"></span><button class="office-btn primary" id="imgCropRun" ${current?'':'disabled'}>Cắt và tải ảnh</button></div></div>`);
  const ratioSelect=h.querySelector('#imgCropRatio'),formatSelect=h.querySelector('#imgCropFormat');
  ratioSelect.value=ratio;
  formatSelect.value=state.settings.imageCropFormat||'image/jpeg';
  h.querySelector('#imgCropSource').onchange=e=>{state.imageCrop.fileId=e.target.value;state.imageCrop.selection=null;renderImageTool()};
  ratioSelect.onchange=()=>{state.imageCrop.ratio=ratioSelect.value;state.imageCrop.selection=null;renderImageCropPreview(h)};
  formatSelect.onchange=()=>{state.settings.imageCropFormat=formatSelect.value;saveSettings()};
  h.querySelector('#imgCropReset').onclick=()=>{state.imageCrop.selection=null;renderImageCropPreview(h)};
  h.querySelector('#imgCropRun').onclick=runImageCrop;
  if(current)renderImageCropPreview(h);
}
function cropRatioValue(value){if(!value||value==='free')return null;const [a,b]=value.split(':').map(Number);return a>0&&b>0?a/b:null}
function defaultCropSelection(width,height,ratioValue){if(!ratioValue)return{x:0,y:0,w:width,h:height};let w=width,h=w/ratioValue;if(h>height){h=height;w=h*ratioValue}return{x:(width-w)/2,y:(height-h)/2,w,h}}
function syncCropSelectionOverlay(stage){const c=stage.querySelector('canvas'),box=stage.querySelector('.image-crop-selection'),info=mainHost.querySelector('#imgCropInfo'),sel=state.imageCrop.selection,sw=+stage.dataset.sourceWidth,sh=+stage.dataset.sourceHeight;if(!sel||!sw||!sh)return;const sx=c.width/sw,sy=c.height/sh;box.style.left=`${sel.x*sx}px`;box.style.top=`${sel.y*sy}px`;box.style.width=`${sel.w*sx}px`;box.style.height=`${sel.h*sy}px`;info.value=`${Math.max(1,Math.round(sel.w))} × ${Math.max(1,Math.round(sel.h))} px`}
async function renderImageCropPreview(h){
  const wrap=h.querySelector('#imgCropPreviewWrap');if(!wrap)return;
  const item=state.imageFiles.find(x=>x.id===state.imageCrop.fileId)||state.imageFiles[0];if(!item){wrap.innerHTML='<div class="office-empty">Hãy chọn ảnh ở phía trên.</div>';return}
  try{
    wrap.innerHTML='<div class="office-empty">Đang tạo xem trước…</div>';
    const bitmap=await createImageBitmap(item.file),sw=bitmap.width,sh=bitmap.height,maxW=Math.max(280,Math.min(920,(h.clientWidth||900)-56)),maxH=520,scale=Math.min(maxW/sw,maxH/sh,1),dw=Math.max(1,Math.round(sw*scale)),dh=Math.max(1,Math.round(sh*scale));
    wrap.innerHTML='<div class="image-crop-stage"><canvas></canvas><div class="image-crop-selection"><i></i><i></i><i></i><i></i></div></div><div class="image-crop-hint">Kéo chuột từ một góc sang góc đối diện để chọn vùng cắt.</div>';
    const stage=wrap.querySelector('.image-crop-stage'),canvas=stage.querySelector('canvas'),ctx=canvas.getContext('2d');canvas.width=dw;canvas.height=dh;ctx.drawImage(bitmap,0,0,dw,dh);bitmap.close();stage.style.width=`${dw}px`;stage.style.height=`${dh}px`;stage.dataset.sourceWidth=sw;stage.dataset.sourceHeight=sh;
    if(!state.imageCrop.selection)state.imageCrop.selection=defaultCropSelection(sw,sh,cropRatioValue(state.imageCrop.ratio));
    syncCropSelectionOverlay(stage);bindCropStage(stage);
  }catch(error){wrap.innerHTML=`<div class="office-empty">Không thể đọc ảnh: ${escapeHtml(error.message)}</div>`}
}
function bindCropStage(stage){
  const sw=+stage.dataset.sourceWidth,sh=+stage.dataset.sourceHeight,ratio=cropRatioValue(state.imageCrop.ratio);let start=null,previous=null;
  const point=event=>{const rect=stage.getBoundingClientRect();return{x:Math.max(0,Math.min(sw,(event.clientX-rect.left)/rect.width*sw)),y:Math.max(0,Math.min(sh,(event.clientY-rect.top)/rect.height*sh))}};
  const build=(a,p)=>{let dx=p.x-a.x,dy=p.y-a.y;if(!ratio)return{x:Math.min(a.x,p.x),y:Math.min(a.y,p.y),w:Math.abs(dx),h:Math.abs(dy)};const signX=dx<0?-1:1,signY=dy<0?-1:1,maxW=signX>0?sw-a.x:a.x,maxH=signY>0?sh-a.y:a.y;let w=Math.abs(dx),h=Math.abs(dy);if(!w&&!h)return{x:a.x,y:a.y,w:0,h:0};if(!h||w/h>ratio)h=w/ratio;else w=h*ratio;w=Math.min(w,maxW,maxH*ratio);h=w/ratio;return{x:signX>0?a.x:a.x-w,y:signY>0?a.y:a.y-h,w,h}};
  stage.onpointerdown=event=>{if(event.button!==0)return;event.preventDefault();previous=state.imageCrop.selection?{...state.imageCrop.selection}:null;start=point(event);stage.setPointerCapture?.(event.pointerId);state.imageCrop.selection={x:start.x,y:start.y,w:0,h:0};syncCropSelectionOverlay(stage)};
  stage.onpointermove=event=>{if(!start)return;state.imageCrop.selection=build(start,point(event));syncCropSelectionOverlay(stage)};
  const done=event=>{if(!start)return;const sel=state.imageCrop.selection;if(!sel||sel.w<3||sel.h<3)state.imageCrop.selection=previous||defaultCropSelection(sw,sh,ratio);start=null;try{stage.releasePointerCapture?.(event.pointerId)}catch{}syncCropSelectionOverlay(stage)};
  stage.onpointerup=done;stage.onpointercancel=done;
}
async function runImageCrop(){
  try{
    const item=state.imageFiles.find(x=>x.id===state.imageCrop.fileId)||state.imageFiles[0];if(!item)throw new Error('Hãy chọn một ảnh để cắt.');
    startBusy('Đang cắt ảnh…');const bitmap=await createImageBitmap(item.file),raw=state.imageCrop.selection||{x:0,y:0,w:bitmap.width,h:bitmap.height},sx=Math.max(0,Math.min(bitmap.width-1,Math.round(raw.x))),sy=Math.max(0,Math.min(bitmap.height-1,Math.round(raw.y))),cw=Math.max(1,Math.min(bitmap.width-sx,Math.round(raw.w))),ch=Math.max(1,Math.min(bitmap.height-sy,Math.round(raw.h))),format=mainHost.querySelector('#imgCropFormat').value,quality=Math.max(10,Math.min(100,+mainHost.querySelector('#imgCropQuality').value||100)),canvas=document.createElement('canvas');canvas.width=cw;canvas.height=ch;const ctx=canvas.getContext('2d');if(format==='image/jpeg'){ctx.fillStyle='#ffffff';ctx.fillRect(0,0,cw,ch)}ctx.drawImage(bitmap,sx,sy,cw,ch,0,0,cw,ch);bitmap.close();const blob=await new Promise(resolve=>canvas.toBlob(resolve,format,quality/100));canvas.width=canvas.height=1;if(!blob)throw new Error('Không thể tạo ảnh sau khi cắt.');state.settings.imageCropFormat=format;state.settings.imageCropQuality=quality;saveSettings();const ext={'image/jpeg':'.jpg','image/png':'.png','image/webp':'.webp'}[format];await saveBlob(blob,`${baseName(item.file.name)}_crop${ext}`,'Ảnh đã cắt');endBusy(`Đã cắt ảnh ${cw} × ${ch} px.`)
...
// ===== TẠO ẢNH DÀI TỪ NỘI DUNG DÁN =====
const LONG_IMAGE_BASE_WIDTH=720;
const LONG_IMAGE_MAX_HEIGHT=30000;
const LONG_IMAGE_MAX_PIXELS=48000000;
const LONG_IMAGE_MAX_EMBED_PIXELS=12000000;
const LONG_IMAGE_MAX_EMBED_SIDE=8192;
let longComposerRange=null;
let longComposerSelectedImage=null;
const LONG_IMAGE_EXPORT_CSS=`
  *{box-sizing:border-box}
  .long-export-root{width:${LONG_IMAGE_BASE_WIDTH}px;min-height:220px;padding:48px;background:#fff;color:#17241e;font:28px/1.55 Arial,"Segoe UI",sans-serif;overflow-wrap:anywhere;word-break:normal}
  .long-export-root p{margin:0 0 1em}.long-export-root h1{font-size:1.85em;line-height:1.2;margin:.2em 0 .65em}.long-export-root h2{font-size:1.5em;line-height:1.25;margin:1em 0 .55em}.long-export-root h3{font-size:1.25em;line-height:1.3;margin:.9em 0 .5em}
  .long-export-root h4,.long-export-root h5,.long-export-root h6{font-size:1.08em;line-height:1.35;margin:.8em 0 .45em}.long-export-root ul,.long-export-root ol{margin:.35em 0 1em;padding-left:1.5em}.long-export-root li{margin:.28em 0}
  .long-export-root img{display:block;max-width:100%;height:auto;margin:1em auto}.long-export-root figure{margin:1em 0}.long-export-root figcaption{font-size:.82em;color:#607069;text-align:center;margin-top:.4em}
  .long-export-root blockquote{margin:1em 0;padding:.75em 1em;border-left:6px solid #58aa83;background:#eef8f2;color:#355247}.long-export-root pre{white-space:pre-wrap;background:#f1f4f2;border-radius:10px;padding:.8em;font:22px/1.5 Consolas,monospace}.long-export-root code{font-family:Consolas,monospace}
  .long-export-root table{width:100%;border-collapse:collapse;margin:1em 0;font-size:.88em}.long-export-root th,.long-export-root td{border:1px solid #bfcac4;padding:.5em .6em;text-align:left;vertical-align:top}.long-export-root th{background:#eaf4ee}.long-export-root hr{border:0;border-top:2px solid #d9e2dd;margin:1.2em 0}.long-export-root a{color:#176c9c;text-decoration:underline}
`;

function renderLongImageComposer(h){
  const c=state.imageComposer;
  h.innerHTML=`<div class="office-card long-image-compose-card">
    <div class="office-card-title-row"><div><h4>Tạo ảnh dài từ Word hoặc ChatGPT</h4><p class="office-card-note">Dán trực tiếp chữ và hình vào khung dưới. Nội dung chỉ nằm trong bộ nhớ trình duyệt và sẽ không được tải lên máy chủ.</p></div><span class="office-recommend-badge">Mặc định tối ưu đọc trên điện thoại</span></div>
    <div class="long-image-formatbar" aria-label="Định dạng nội dung">
      <button type="button" data-long-cmd="bold" title="In đậm"><b>B</b></button><button type="button" data-long-cmd="italic" title="In nghiêng"><i>I</i></button><button type="button" data-long-cmd="underline" title="Gạch chân"><u>U</u></button>
      <select id="longBlockFormat" title="Kiểu đoạn"><option value="p">Đoạn thường</option><option value="h1">Tiêu đề lớn</option><option value="h2">Tiêu đề</option><option value="h3">Tiêu đề nhỏ</option></select>
      <button type="button" data-long-cmd="insertUnorderedList" title="Danh sách chấm">• Danh sách</button><button type="button" data-long-cmd="insertOrderedList" title="Danh sách số">1. Danh sách</button>
      <button type="button" data-long-cmd="justifyLeft" title="Căn trái">≡←</button><button type="button" data-long-cmd="justifyCenter" title="Căn giữa">≡</button><button type="button" data-long-cmd="justifyRight" title="Căn phải">→≡</button>
      <label class="long-color-control" title="Màu chữ"><span>A</span><input id="longTextColor" type="color" value="#17241e"></label>
      <button type="button" id="longUndo" title="Hoàn tác">↶</button><button type="button" id="longRedo" title="Làm lại">↷</button><button type="button" id="longClearFormat">Bỏ định dạng</button>
    </div>
    <div class="long-image-toolbar"><button class="office-btn" type="button" id="longAddImages">Chèn ảnh</button><span>Ảnh đang chọn:</span><button class="office-btn" type="button" data-long-image-width="50">50%</button><button class="office-btn" type="button" data-long-image-width="75">75%</button><button class="office-btn" type="button" data-long-image-width="100">100%</button><span class="spacer"></span><button class="office-btn danger" type="button" id="longClearAll">Xóa nội dung</button></div>
    <div class="long-image-editor-shell"><div id="longImageEditor" class="long-image-editor" contenteditable="true" spellcheck="true" data-placeholder="Bấm vào đây rồi dán nội dung từ Word hoặc ChatGPT…"></div></div>
    <div class="office-chatgpt-note">Mẹo: nếu hình trong nội dung Word không đi theo khi dán, hãy đặt con trỏ đúng vị trí rồi bấm <b>Chèn ảnh</b>. Bấm vào một hình để đổi nhanh chiều rộng 50% / 75% / 100%.</div>
  </div>
  <div class="office-card"><h4>Xuất ảnh</h4><div class="office-grid three">
    <label class="office-field"><span>Chiều rộng ảnh</span><select id="longOutputWidth"><option value="1080">1080 px · khuyến nghị</option><option value="1200">1200 px</option><option value="1440">1440 px · rõ hơn, file lớn</option></select></label>
    <label class="office-field"><span>Định dạng</span><select id="longOutputFormat"><option value="image/png">PNG · chữ sắc nét</option><option value="image/jpeg">JPG · file nhẹ hơn</option></select></label>
    <label class="office-field"><span>Chất lượng JPG %</span><input id="longOutputQuality" type="number" min="60" max="100" value="92"></label>
    <label class="office-field"><span>Màu nền</span><input id="longOutputBg" type="color" value="#ffffff"></label>
    <label class="office-field"><span>Tên file</span><input id="longOutputName" value="Noi_dung_dai"></label>
    <label class="office-field"><span>Khả năng đọc</span><span class="long-normalize-option"><input id="longNormalizeText" type="checkbox" checked> Chuẩn hóa cỡ chữ để mở ảnh vừa bề ngang vẫn đọc rõ</span></label>
  </div><div class="office-toolbar"><span class="office-card-note" id="longImageEstimate">Ảnh sẽ tự kéo dài theo toàn bộ nội dung.</span><span class="spacer"></span><button class="office-btn primary" type="button" id="longImageExport">Tạo và tải PNG</button></div></div>
  <div id="longImageResult"></div>`;
  const editor=h.querySelector('#longImageEditor');editor.innerHTML=c.html;
  h.querySelector('#longOutputWidth').value=String(c.width);h.querySelector('#longOutputFormat').value=c.format;h.querySelector('#longOutputQuality').value=String(c.quality);h.querySelector('#longOutputBg').value=c.bg;h.querySelector('#longOutputName').value=c.name;h.querySelector('#longNormalizeText').checked=c.normalizeText;
  const sync=()=>{c.html=longComposerHtml(editor);updateLongImageEstimate(editor)};
  editor.addEventListener('input',sync);editor.addEventListener('paste',event=>handleLongComposerPaste(event,editor));editor.addEventListener('mouseup',()=>rememberLongComposerRange(editor));editor.addEventListener('keyup',()=>rememberLongComposerRange(editor));editor.addEventListener('focus',()=>rememberLongComposerRange(editor));
  editor.addEventListener('click',event=>{const image=event.target.closest('img');selectLongComposerImage(editor,image);rememberLongComposerRange(editor)});
  editor.addEventListener('dragover',event=>{if([...event.dataTransfer.items].some(item=>item.type.startsWith('image/')))event.preventDefault()});editor.addEventListener('drop',event=>handleLongComposerDrop(event,editor));
  h.querySelectorAll('[data-long-cmd]').forEach(button=>{button.onmousedown=event=>event.preventDefault();button.onclick=()=>applyLongComposerCommand(editor,button.dataset.longCmd)});
  h.querySelector('#longBlockFormat').onchange=event=>applyLongComposerCommand(editor,'formatBlock',event.target.value);
  h.querySelector('#longTextColor').oninput=event=>applyLongComposerCommand(editor,'foreColor',event.target.value);
  h.querySelector('#longUndo').onclick=()=>applyLongComposerCommand(editor,'undo');h.querySelector('#longRedo').onclick=()=>applyLongComposerCommand(editor,'redo');h.querySelector('#longClearFormat').onclick=()=>applyLongComposerCommand(editor,'removeFormat');
  h.querySelector('#longAddImages').onclick=async()=>insertLongComposerImages(editor,await fileInput('image/*',true));
  h.querySelectorAll('[data-long-image-width]').forEach(button=>button.onclick=()=>resizeLongComposerImage(editor,+button.dataset.longImageWidth));
  h.querySelector('#longClearAll').onclick=()=>{if(editor.innerHTML.trim()&&!confirm('Xóa toàn bộ nội dung đang soạn?'))return;editor.innerHTML='';selectLongComposerImage(editor,null);sync();editor.focus()};
  h.querySelector('#longOutputWidth').onchange=event=>{c.width=+event.target.value;updateLongImageEstimate(editor)};h.querySelector('#longOutputFormat').onchange=event=>{c.format=event.target.value;syncLongExportButton(h)};h.querySelector('#longOutputQuality').oninput=event=>c.quality=Math.max(60,Math.min(100,+event.target.value||92));h.querySelector('#longOutputBg').oninput=event=>c.bg=event.target.value;h.querySelector('#longOutputName').oninput=event=>c.name=event.target.value;h.querySelector('#longNormalizeText').onchange=event=>c.normalizeText=event.target.checked;
  h.querySelector('#longImageExport').onclick=()=>runLongImageExport(editor,h.querySelector('#longImageResult'));
  syncLongExportButton(h);updateLongImageEstimate(editor);
}

function syncLongExportButton(host){const format=host.querySelector('#longOutputFormat'),quality=host.querySelector('#longOutputQuality'),button=host.querySelector('#longImageExport');quality.disabled=format.value==='image/png';button.textContent=format.value==='image/png'?'Tạo và tải PNG':'Tạo và tải JPG'}
function longComposerHtml(editor){const clone=editor.cloneNode(true);clone.querySelectorAll('.long-image-selected').forEach(image=>image.classList.remove('long-image-selected'));return clone.innerHTML}
function rememberLongComposerRange(editor){const selection=getSelection();if(selection?.rangeCount&&editor.contains(selection.getRangeAt(0).commonAncestorContainer))longComposerRange=selection.getRangeAt(0).cloneRange()}
function restoreLongComposerRange(editor){const selection=getSelection();selection.removeAllRanges();if(longComposerRange&&editor.contains(longComposerRange.commonAncestorContainer))selection.addRange(longComposerRange);else{const range=document.createRange();range.selectNodeContents(editor);range.collapse(false);selection.addRange(range)}}
function applyLongComposerCommand(editor,command,value=null){editor.focus();restoreLongComposerRange(editor);document.execCommand(command,false,value);rememberLongComposerRange(editor);state.imageComposer.html=longComposerHtml(editor);updateLongImageEstimate(editor)}
function selectLongComposerImage(editor,image){editor.querySelectorAll('.long-image-selected').forEach(item=>item.classList.remove('long-image-selected'));longComposerSelectedImage=image&&editor.contains(image)?image:null;if(longComposerSelectedImage)longComposerSelectedImage.classList.add('long-image-selected')}
function resizeLongComposerImage(editor,width){if(!longComposerSelectedImage||!editor.contains(longComposerSelectedImage)){setStatus('Hãy bấm chọn một hình trong nội dung trước.');return}longComposerSelectedImage.style.width=`${width}%`;longComposerSelectedImage.style.height='auto';state.imageComposer.html=longComposerHtml(editor);updateLongImageEstimate(editor)}
function updateLongImageEstimate(editor){const estimate=mainHost.querySelector('#longImageEstimate');if(!estimate)return;const scale=state.imageComposer.width/LONG_IMAGE_BASE_WIDTH,height=Math.max(220,editor.scrollHeight),outHeight=Math.round(height*scale);estimate.textContent=`Ước tính: ${state.imageComposer.width.toLocaleString('vi-VN')} × ${outHeight.toLocaleString('vi-VN')} px · chiều cao tự động theo nội dung.`}

function sanitizeLongComposerHtml(html){
  const template=document.createElement('template');template.innerHTML=String(html||'');
  const comments=document.createTreeWalker(template.content,NodeFilter.SHOW_COMMENT);let comment;const commentNodes=[];while(comment=comments.nextNode())commentNodes.push(comment);commentNodes.forEach(node=>node.remove());
  template.content.querySelectorAll('script,style,link,meta,base,iframe,object,embed,form,input,button,textarea,select,canvas,svg,video,audio,source').forEach(node=>node.remove());
  [...template.content.querySelectorAll('picture')].forEach(element=>element.replaceWith(...element.childNodes));
  [...template.content.querySelectorAll('*')].filter(element=>element.tagName.includes(':')).forEach(element=>element.replaceWith(...element.childNodes));
  template.content.querySelectorAll('*').forEach(element=>{
    [...element.attributes].forEach(attribute=>{const name=attribute.name.toLowerCase(),value=attribute.value,remoteAttribute=['href','srcset','sizes','background','poster','usemap','formaction','manifest','ping'].includes(name);if(name.startsWith('on')||name.includes(':')||name.startsWith('xmlns')||['id','class','srcdoc','contenteditable','crossorigin','referrerpolicy'].includes(name)||remoteAttribute||(name==='src'&&element.tagName!=='IMG'))element.removeAttribute(attribute.name);else if(name==='src'&&/^\s*javascript:/i.test(value))element.removeAttribute(attribute.name)});
    for(const property of Array.from(element.style)){const value=element.style.getPropertyValue(property);if(['position','left','right','top','bottom','z-index','transform','behavior','filter'].includes(property)||/url\s*\(|expression\s*\(/i.test(value))element.style.removeProperty(property)}
    if(!element.getAttribute('style')?.trim())element.removeAttribute('style');
    if(element.tagName==='IMG'){element.removeAttribute('loading');element.removeAttribute('decoding');const src=element.getAttribute('src')||'';if(src&&!/^(data:image\/|blob:|https?:\/\/)/i.test(src))element.removeAttribute('src')}
    if(element.tagName==='A')element.removeAttribute('target');
  });
  return template.innerHTML;
}
function plainTextToLongHtml(text){return String(text||'').replace(/\r/g,'').split(/\n{2,}/).map(block=>`<p>${escapeHtml(block).replace(/\n/g,'<br>')}</p>`).join('')}
function blobToDataUrl(blob){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(reader.error||new Error('Không đọc được hình ảnh.'));reader.readAsDataURL(blob)})}
async function handleLongComposerPaste(event,editor){
  event.preventDefault();rememberLongComposerRange(editor);const range=longComposerRange?.cloneRange(),clipboard=event.clipboardData,html=clipboard.getData('text/html'),text=clipboard.getData('text/plain'),files=[...clipboard.items].filter(item=>item.kind==='file'&&item.type.startsWith('image/')).map(item=>item.getAsFile()).filter(Boolean);
  try{const dataUrls=await Promise.all(files.map(blobToDataUrl)),template=document.createElement('template');template.innerHTML=sanitizeLongComposerHtml(html||plainTextToLongHtml(text));const images=[...template.content.querySelectorAll('img')];let used=0;for(const image of images){if(used>=dataUrls.length)break;if(!/^data:image\//i.test(image.getAttribute('src')||'')){image.src=dataUrls[used++];}}
    for(;used<dataUrls.length;used++){const image=document.createElement('img');image.src=dataUrls[used];image.alt='Hình đã dán';template.content.append(image)}
    insertLongComposerHtml(editor,template.innerHTML,range);state.imageComposer.html=longComposerHtml(editor);updateLongImageEstimate(editor);setStatus(`Đã dán nội dung${files.length?` và ${files.length} hình`:''}.`);
  }catch(error){setStatus(`Không thể dán: ${error.message}`)}
}
function handleLongComposerDrop(event,editor){const files=[...event.dataTransfer.files].filter(file=>file.type.startsWith('image/'));if(!files.length)return;event.preventDefault();rememberLongComposerRange(editor);insertLongComposerImages(editor,files)}
async function insertLongComposerImages(editor,files){if(!files.length)return;try{const urls=await Promise.all(files.map(blobToDataUrl));insertLongComposerHtml(editor,urls.map(url=>`<img src="${url}" alt="Hình đã chèn">`).join(''),longComposerRange?.cloneRange());state.imageComposer.html=longComposerHtml(editor);updateLongImageEstimate(editor);setStatus(`Đã chèn ${urls.length} hình.`)}catch(error){setStatus(`Không thể chèn hình: ${error.message}`)}}
function insertLongComposerHtml(editor,html,range){editor.focus();let target=range;if(!target||!editor.contains(target.commonAncestorContainer)){target=document.createRange();target.selectNodeContents(editor);target.collapse(false)}target.deleteContents();const fragment=target.createContextualFragment(html),last=fragment.lastChild;target.insertNode(fragment);if(last){target.setStartAfter(last);target.collapse(true);const selection=getSelection();selection.removeAllRanges();selection.addRange(target);longComposerRange=target.cloneRange()}editor.dispatchEvent(new InputEvent('input',{bubbles:true,inputType:'insertFromPaste',data:null}))}

function normalizeLongExportText(root){root.querySelectorAll('*').forEach(element=>{for(const property of ['font','font-size','font-family','line-height','letter-spacing'])element.style.removeProperty(property);if(!element.getAttribute('style')?.trim())element.removeAttribute('style')})}
async function rasterizeLongExportImage(image,index){
  await waitForLongImage(image,index);const naturalWidth=image.naturalWidth,naturalHeight=image.naturalHeight;if(!naturalWidth||!naturalHeight)throw new Error(`Không đọc được kích thước hình thứ ${index}.`);const displayedWidth=image.getBoundingClientRect().width,hasAuthoredWidth=!!(image.style.width||image.getAttribute('width')),scale=Math.min(1,LONG_IMAGE_MAX_EMBED_SIDE/naturalWidth,LONG_IMAGE_MAX_EMBED_SIDE/naturalHeight,Math.sqrt(LONG_IMAGE_MAX_EMBED_PIXELS/(naturalWidth*naturalHeight))),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(naturalWidth*scale));canvas.height=Math.max(1,Math.round(naturalHeight*scale));
  try{const context=canvas.getContext('2d');context.drawImage(image,0,0,canvas.width,canvas.height);context.getImageData(0,0,1,1);image.src=canvas.toDataURL('image/png')}catch(error){throw new Error(`Hình thứ ${index} chứa nguồn ngoài mà trình duyệt không cho xuất. Hãy lưu hình về máy rồi chèn lại bằng nút “Chèn ảnh”.`)}finally{canvas.width=canvas.height=1}
  image.removeAttribute('crossorigin');image.removeAttribute('referrerpolicy');if(!hasAuthoredWidth&&displayedWidth)image.style.width=`${displayedWidth}px`;await waitForLongImage(image,index);
}
async function inlineLongExportImages(root){const images=[...root.querySelectorAll('img')];for(let i=0;i<images.length;i++){assertNotAborted();const image=images[i],src=image.getAttribute('src')||'';if(!src)throw new Error(`Hình thứ ${i+1} chưa được nhúng. Hãy chèn lại bằng nút “Chèn ảnh”.`);image.removeAttribute('srcset');image.removeAttribute('sizes');if(!src.startsWith('data:image/')){try{const response=await fetch(src,{mode:'cors',credentials:'omit'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const blob=await response.blob();if(!blob.type.startsWith('image/'))throw new Error('Tệp không phải hình ảnh');image.src=await blobToDataUrl(blob)}catch{throw new Error(`Hình thứ ${i+1} lấy từ nguồn ngoài không cho phép xuất. Hãy lưu hình về máy rồi chèn lại.`)}}await rasterizeLongExportImage(image,i+1)}}
function waitForLongImage(image,index){if(image.complete&&image.naturalWidth)return Promise.resolve();return new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error(`Hình thứ ${index} tải quá lâu.`)),15000);image.onload=()=>{clearTimeout(timer);resolve()};image.onerror=()=>{clearTimeout(timer);reject(new Error(`Không đọc được hình thứ ${index}.`))}})}

function longCssPx(value){const number=parseFloat(value);return Number.isFinite(number)?number:0}
function longTransparentColor(color){return !color||color==='transparent'||/^rgba\([^)]*,\s*0(?:\.0+)?\)$/i.test(color)||/\/\s*0\s*\)$/i.test(color)}
function longRelativeRects(element,origin){return [...element.getClientRects()].map(rect=>({left:rect.left-origin.left,top:rect.top-origin.top,width:rect.width,height:rect.height})).filter(rect=>rect.width>.01&&rect.height>.01)}
function drawLongElementBoxes(element,context,origin,style){
  const rects=longRelativeRects(element,origin),background=style.backgroundColor;if(!longTransparentColor(background)){context.fillStyle=background;for(const rect of rects)context.fillRect(rect.left,rect.top,rect.width,rect.height)}
  const sides=[['Top','top'],['Right','right'],['Bottom','bottom'],['Left','left']];for(const [cssSide,side] of sides){const width=longCssPx(style[`border${cssSide}Width`]);if(!width||style[`border${cssSide}Style`]==='none'||longTransparentColor(style[`border${cssSide}Color`]))continue;context.fillStyle=style[`border${cssSide}Color`];for(const rect of rects){if(side==='top')context.fillRect(rect.left,rect.top,rect.width,width);if(side==='right')context.fillRect(rect.left+rect.width-width,rect.top,width,rect.height);if(side==='bottom')context.fillRect(rect.left,rect.top+rect.height-width,rect.width,width);if(side==='left')context.fillRect(rect.left,rect.top,width,rect.height)}}
}
function setLongCanvasTextStyle(context,style){context.font=`${style.fontStyle||'normal'} ${style.fontVariant||'normal'} ${style.fontWeight||'400'} ${style.fontSize||'16px'} ${style.fontFamily||'Arial'}`;context.fillStyle=style.color||'#17241e';context.textAlign='left';context.textBaseline='alphabetic';context.direction=style.direction||'ltr';if('letterSpacing'in context)context.letterSpacing=style.letterSpacing||'0px'}
function longTextBaseline(context,top,height,fontSize){const metrics=context.measureText('Mg'),ascent=metrics.actualBoundingBoxAscent||fontSize*.8,descent=metrics.actualBoundingBoxDescent||fontSize*.2;return top+Math.max(0,(height-ascent-descent)/2)+ascent}
function drawLongTextFragment(context,fragment,style){
  setLongCanvasTextStyle(context,style);const fontSize=longCssPx(style.fontSize)||16,baseline=longTextBaseline(context,fragment.top,fragment.height,fontSize);context.fillText(fragment.text,fragment.left,baseline);const decoration=style.textDecorationLine||'';if(decoration.includes('underline')||decoration.includes('line-through')){const width=context.measureText(fragment.text).width;context.strokeStyle=style.textDecorationColor||style.color;context.lineWidth=Math.max(1,fontSize/18);if(decoration.includes('underline')){context.beginPath();context.moveTo(fragment.left,baseline+Math.max(1,fontSize*.08));context.lineTo(fragment.left+width,baseline+Math.max(1,fontSize*.08));context.stroke()}if(decoration.includes('line-through')){context.beginPath();context.moveTo(fragment.left,baseline-fontSize*.3);context.lineTo(fragment.left+width,baseline-fontSize*.3);context.stroke()}}
}
async function drawLongTextNode(node,context,origin,style,progress){
  const text=node.nodeValue||'';if(!text)return;const range=document.createRange();let fragment=null;const flush=()=>{if(fragment){drawLongTextFragment(context,fragment,style);fragment=null}};for(let index=0;index<text.length;index++){range.setStart(node,index);range.setEnd(node,index+1);const rect=range.getBoundingClientRect();if(rect.width>.01&&rect.height>.01){const left=rect.left-origin.left,top=rect.top-origin.top,sameLine=fragment&&Math.abs(fragment.top-top)<.75&&left<=fragment.right+3;if(!sameLine)flush();if(!fragment)fragment={text:'',left,top,height:rect.height,right:left};fragment.text+=text[index];fragment.height=Math.max(fragment.height,rect.height);fragment.right=Math.max(fragment.right,left+rect.width)}if(++progress.count%700===0){assertNotAborted();await sleep()}}flush();range.detach?.();
}
function drawLongListMarker(element,context,origin,style){
  const parent=element.parentElement;if(!parent||!['UL','OL'].includes(parent.tagName)||style.listStyleType==='none')return;const rect=element.getBoundingClientRect();if(!rect.width||!rect.height)return;let marker='•';if(parent.tagName==='OL'){const items=[...parent.children].filter(child=>child.tagName==='LI'),start=Number(parent.getAttribute('start'))||1;marker=`${start+Math.max(0,items.indexOf(element))}.`}else if(style.listStyleType==='circle')marker='◦';else if(style.listStyleType==='square')marker='▪';setLongCanvasTextStyle(context,style);context.textAlign='right';const fontSize=longCssPx(style.fontSize)||16,lineHeight=longCssPx(style.lineHeight)||fontSize*1.2,top=rect.top-origin.top,baseline=longTextBaseline(context,top,lineHeight,fontSize);context.fillText(marker,rect.left-origin.left-fontSize*.35,baseline);
}
async function drawLongDomNode(node,context,origin,progress,parentOpacity=1){
  if(node.nodeType===Node.TEXT_NODE){const parent=node.parentElement;if(!parent)return;const style=getComputedStyle(parent);if(style.display==='none'||style.visibility==='hidden')return;context.save();context.globalAlpha=parentOpacity;await drawLongTextNode(node,context,origin,style,progress);context.restore();return}
  if(node.nodeType!==Node.ELEMENT_NODE||['STYLE','SCRIPT','NOSCRIPT','TEMPLATE'].includes(node.tagName))return;const style=getComputedStyle(node);if(style.display==='none'||style.visibility==='hidden')return;const ownOpacity=Number.parseFloat(style.opacity),opacity=parentOpacity*(Number.isFinite(ownOpacity)?ownOpacity:1);context.save();context.globalAlpha=opacity;drawLongElementBoxes(node,context,origin,style);if(node.tagName==='LI')drawLongListMarker(node,context,origin,style);if(node.tagName==='IMG'){const rect=node.getBoundingClientRect();if(rect.width&&rect.height)context.drawImage(node,rect.left-origin.left,rect.top-origin.top,rect.width,rect.height);context.restore();return}context.restore();for(const child of node.childNodes)await drawLongDomNode(child,context,origin,progress,opacity);
}
async function renderLongDomCanvas(root,sourceWidth,sourceHeight,outputWidth,outputHeight){
  const canvas=document.createElement('canvas');canvas.width=outputWidth;canvas.height=outputHeight;const context=canvas.getContext('2d',{alpha:false}),scale=outputWidth/sourceWidth;context.setTransform(scale,0,0,scale,0,0);context.fillStyle=state.imageComposer.bg;context.fillRect(0,0,sourceWidth,sourceHeight);const origin=root.getBoundingClientRect(),progress={count:0};for(const child of root.childNodes)await drawLongDomNode(child,context,origin,progress);context.setTransform(1,0,0,1,0,0);return canvas;
}

async function runLongImageExport(editor,resultHost){
  let measureHost=null;
  try{
    state.imageComposer.html=longComposerHtml(editor);if(!editor.textContent.trim()&&!editor.querySelector('img'))throw new Error('Hãy dán hoặc nhập nội dung trước khi tạo ảnh.');startBusy('Đang chuẩn bị nội dung và hình ảnh…');
    measureHost=document.createElement('div');measureHost.className='long-image-measure-host';const root=document.createElement('div');root.className='long-export-root';root.style.backgroundColor=state.imageComposer.bg;const style=document.createElement('style');style.textContent=LONG_IMAGE_EXPORT_CSS;const content=document.createElement('div');content.className='long-export-content';content.innerHTML=sanitizeLongComposerHtml(state.imageComposer.html);content.querySelectorAll('.long-image-selected').forEach(image=>image.classList.remove('long-image-selected'));if(state.imageComposer.normalizeText)normalizeLongExportText(content);root.append(style,content);measureHost.append(root);document.body.append(measureHost);
    await inlineLongExportImages(content);if(document.fonts?.ready)await document.fonts.ready;assertNotAborted();await sleep();
    const sourceWidth=LONG_IMAGE_BASE_WIDTH,sourceHeight=Math.max(220,Math.ceil(root.scrollHeight)),outputWidth=state.imageComposer.width,outputHeight=Math.ceil(sourceHeight*outputWidth/sourceWidth);
    if(outputHeight>LONG_IMAGE_MAX_HEIGHT||outputWidth*outputHeight>LONG_IMAGE_MAX_PIXELS)throw new Error(`Nội dung quá dài (${outputHeight.toLocaleString('vi-VN')} px). Hãy chia thành hai ảnh để trình duyệt không hết bộ nhớ.`);
    setStatus(`Đang dựng ảnh ${outputWidth.toLocaleString('vi-VN')} × ${outputHeight.toLocaleString('vi-VN')} px…`,45);
    const canvas=await renderLongDomCanvas(root,sourceWidth,sourceHeight,outputWidth,outputHeight),context=canvas.getContext('2d',{alpha:false});assertNotAborted();setStatus('Đang mã hóa tệp ảnh…',82);
    const format=state.imageComposer.format,quality=state.imageComposer.quality/100;let blob;try{context.getImageData(0,0,1,1);blob=await new Promise((resolve,reject)=>{try{canvas.toBlob(resolve,format,quality)}catch(error){reject(error)}})}catch(error){if(error?.name==='SecurityError')throw new Error('Một hình trong nội dung vẫn bị trình duyệt chặn khi xuất. Hãy lưu hình đó về máy rồi chèn lại bằng nút “Chèn ảnh”.');throw error}canvas.width=canvas.height=1;if(!blob)throw new Error('Trình duyệt không thể tạo ảnh ở kích thước này. Hãy giảm chiều rộng hoặc chia nội dung.');
    const ext=format==='image/png'?'.png':'.jpg',filename=safeName(state.imageComposer.name||'Noi_dung_dai',ext);showLongImageResult(resultHost,blob,filename,outputWidth,outputHeight);await saveBlob(blob,filename,format==='image/png'?'Ảnh PNG':'Ảnh JPG');endBusy(`Đã tạo ${filename} · ${outputWidth.toLocaleString('vi-VN')} × ${outputHeight.toLocaleString('vi-VN')} px.`);
  }catch(error){showError(error)}finally{measureHost?.remove()}
}
function showLongImageResult(host,blob,filename,width,height){revokeAllPreviews();const url=URL.createObjectURL(blob);host.innerHTML=`<div class="office-card long-image-result-card"><div class="office-card-title-row"><div><h4>Xem trước ảnh dài</h4><p class="office-card-note">Ảnh được mở vừa toàn bộ chiều ngang; cuộn xuống để xem phần nội dung bên dưới.</p></div><span class="office-recommend-badge">${width.toLocaleString('vi-VN')} × ${height.toLocaleString('vi-VN')} px</span></div><div class="long-image-result-viewport"><img src="${url}" data-object-url="${url}" alt="Xem trước ${escapeHtml(filename)}"></div><div class="office-toolbar"><span class="office-card-note">${escapeHtml(filename)} · ${formatBytes(blob.size)}</span><span class="spacer"></span><button class="office-btn primary" type="button" id="longDownloadAgain">Tải ảnh lại</button></div></div>`;host.querySelector('#longDownloadAgain').onclick=()=>saveBlob(blob,filename,'Ảnh')}
...
function renderExcelInspect(h){h.innerHTML=excelBrowseCard(`<div class="office-card"><h4>Cấu trúc workbook</h4><div id="excelOverview"></div><div class="office-toolbar"><label class="office-check"><input id="excelFormatted" type="checkbox" checked> Xuất formattedValue</label><label class="office-check"><input id="excelHyperlinks" type="checkbox" checked> Hyperlink/comment</label><label class="office-check" title="Chỉ nên bật với workbook nhỏ. Với vùng dùng rất lớn, việc xuất cả ô trống có thể tạo hàng triệu hoặc hàng tỷ ô JSON."><input id="excelEmpty" type="checkbox"> Cả ô trống trong vùng dùng</label><label class="office-check" title="Nếu workbook có VBA/macro, xuất source Module/Class/Sheet/ThisWorkbook. UserForm sẽ hoàn thiện ở bản sau; raw VBA vẫn được giữ để dự phòng."><input id="excelVba" type="checkbox" checked> Kèm VBA / Macro</label><span class="spacer"></span><button class="office-btn" id="excelPreviewJson" ${state.excelFiles.length?'':'disabled'}>Xem trước JSON</button><button class="office-btn primary" id="excelExportJson" ${state.excelFiles.length?'':'disabled'}>Xuất 1 JSON</button></div><div class="office-card-note" style="margin-top:8px">Khi bật “Kèm VBA / Macro”, JSON sẽ chứa source Module/Class/Sheet/ThisWorkbook, trạng thái giải mã từng component, mapping CodeName ↔ tên sheet và raw <code>vbaProject.bin</code> dạng Base64 để dự phòng. UserForm sẽ hoàn thiện ở bản sau. File Excel lớn vẫn được xuất tuần tự theo sheet/ô để hạn chế RAM.</div><pre id="excelJsonPreview" class="office-json-preview" hidden></pre></div>
  ...
  ['#excelFormatted','#excelHyperlinks','#excelEmpty','#excelVba'].forEach(sel=>mainHost.querySelector(sel)?.addEventListener('change',refreshEstimate));
  setTimeout(refreshEstimate,0);
}
function renderExcelOverview(){const host=mainHost.querySelector('#excelOverview');if(!state.excelFiles.length){host.innerHTML='<div class="office-empty">Chưa chọn workbook.</div>';return}const item=state.excelFiles[0],rawBytes=vbaRawByteLength(item.wb?.vbaraw),macroHint=rawBytes?`<span class="office-sheet-chip">VBA/Macro · ${formatBytes(rawBytes)} raw</span>`:/\.(xlsm|xls|xlsb)$/i.test(item.file.name)?'<span class="office-sheet-chip">VBA/Macro · sẽ kiểm tra khi xuất</span>':'';host.innerHTML=`<strong>${escapeHtml(item.file.name)}</strong><div class="office-sheet-list" style="margin-top:9px">${item.wb.SheetNames.map(n=>{const ws=item.wb.Sheets[n],ref=ws['!ref']||'Trống';return `<span class="office-sheet-chip">${escapeHtml(n)} · ${escapeHtml(ref)}</span>`}).join('')}${macroHint}</div>`}
...
function excelJsonOptions(){
  return {
    ...
    includeVba:mainHost.querySelector('#excelVba')?.checked??true
  };
}
function asExcelVbaBytes(value){
...
async function getExcelVbaProjectBytes(item){
  const direct=asExcelVbaBytes(item?.wb?.vbaraw);
  if(direct?.length)return direct;
  const ext=fileExt(item?.file?.name||'');
  if(!['.xlsm','.xlsb','.xlsx'].includes(ext))return null;
  try{
    const JSZip=await ensureZip(),zip=await JSZip.loadAsync(await item.file.arrayBuffer());
    const entry=zip.file('xl/vbaProject.bin')||Object.values(zip.files).find(x=>/vbaProject\.bin$/i.test(x.name));
    return entry?await entry.async('uint8array'):null;
  }catch{return null}
}
async function buildExcelVbaJson(item){
  const legacyMacroSheets=(item?.wb?.SheetNames||[]).filter(name=>item.wb.Sheets?.[name]?.['!type']==='macro');
  const raw=await getExcelVbaProjectBytes(item);
  if(!raw?.length)return {present:legacyMacroSheets.length>0,status:legacyMacroSheets.length?'legacy_macro_sheets_only':'none',modules:[],userForms:[],legacyMacroSheets,warnings:legacyMacroSheets.length?['Workbook có Excel 4.0 Macro Sheet (XLM) nhưng không có VBA blob. Nội dung sheet vẫn nằm trong phần sheets/cells của JSON.']:[]};
...
function buildWorkbookJsonPreview(item,maxCells=1200){
  ...
  const {formatted,links,includeEmpty,includeVba}=excelJsonOptions(),XLSX=globalThis.XLSX;
...
function estimateWorkbookJsonSize(item,sampleLimit=5000){
  ...
  const {formatted,links,includeEmpty,includeVba}=excelJsonOptions(),XLSX=globalThis.XLSX,encoder=new TextEncoder();
  validateExcelEmptyExport(item,includeEmpty);
  ...
  const rawVbaBytes=includeVba?vbaRawByteLength(item.wb?.vbaraw):0;
  const vbaEstimate=rawVbaBytes?Math.ceil(rawVbaBytes*1.55+32768):0;
  return {bytes:Math.ceil(metadata+avg*totalCells+vbaEstimate),cells:totalCells,sampled,vbaBytes:rawVbaBytes};
}
...
async function exportWorkbookJsonSplit(item){
  ...
  try{
    ...
    let vbaFile=null,vbaInfo=null;
    if(includeVba){
      setStatus('Đang đọc VBA / Macro…',59);
      vbaInfo=await buildExcelVbaJson(item);
      vbaFile='vba_project.json';
      await destination.writeFile(vbaFile,JSON.stringify(vbaInfo,null,2));
    }
    await sleep();
    ...
    const manifest={
      ...
      files:{manifest:manifestFile,structure:structureFile,formula_map:formulaMapFile,anomalies:anomaliesFile,...(vbaFile?{vba:vbaFile}:{}),data_parts:outputs.map(p=>p.file)},
      recommended_read_order:[manifestFile,structureFile,formulaMapFile,anomaliesFile,...(vbaFile?[vbaFile]:[]),'relevant_data_parts_only'],
      analysis_status:{...analysis.analysis_status,vba:includeVba?(vbaInfo?.status||'none'):'not_requested'},
      warnings:[...(analysis.warnings||[]),...(vbaInfo?.warnings||[]),...outputs.filter(p=>p.bytes>targetBytes).map(p=>`${p.file} lớn hơn mục tiêu ${targetMb} MB do metadata hoặc một cell đơn lẻ quá lớn.`)],
    ...
    endBusy(`Đã tạo ${outputs.length} DATA PART + manifest/structure/formula_map/anomalies${vbaFile?' + VBA':''}. Tính toàn vẹn: ${integrityResult.status.toUpperCase()}.`);
  }catch(e){
...
async function exportWorkbookJsonStream(item){
  ...
  try{
    ...
    await sink.write('\n  ]');
    if(includeVba){
      setStatus('Đang đọc VBA / Macro…',98);
      const vba=await buildExcelVbaJson(item);
      await sink.write(`,\n  "vba": ${JSON.stringify(vba)}`);
    }
    await sink.write('\n}\n');
    await sink.close();
    sink=null;
    endBusy(`Đã xuất JSON ${item.wb.SheetNames.length} sheet${includeVba?' kèm kiểm tra VBA/Macro':''} mà không tạo một chuỗi JSON dữ liệu sheet khổng lồ trong RAM.`);
  }catch(e){
...
function renderExcelValueSheetChecks(){
  ...
  const ext=fileExt(item.file.name);
  const supported=['.xlsx','.xlsm','.xlsb'].includes(ext);
  ...
  if(compat){
    compat.textContent=supported
      ? (ext==='.xlsb' ? 'XLSB được xử lý bằng SheetJS và ghi lại dưới dạng XLSB. Merge/style/cell format đọc được sẽ được giữ; các tính năng nhị phân đặc thù mà thư viện không hỗ trợ có thể không được bảo toàn tuyệt đối.' : `Đang dùng chế độ bảo toàn package ${ext.toUpperCase()}: chỉ sửa XML của ô công thức, không dựng lại worksheet.`)
      : 'Break Links / Dán Value giữ định dạng hỗ trợ XLSX, XLSM và XLSB. XLS/CSV không dùng chế độ này.';
  }
...
function excelOutputMeta(bookType){
  if(bookType==='xlsb')return {bookType:'xlsb',ext:'.xlsb',mime:'application/vnd.ms-excel.sheet.binary.macroEnabled.12',label:'XLSB'};
  if(bookType==='xlsm')return {bookType:'xlsm',ext:'.xlsm',mime:'application/vnd.ms-excel.sheet.macroEnabled.12',label:'XLSM'};
  return {bookType:'xlsx',ext:'.xlsx',mime:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',label:'XLSX'};
}
function excelSourceBookType(item){const e=fileExt(item?.file?.name||'');return e==='.xlsb'?'xlsb':e==='.xlsm'?'xlsm':'xlsx'}
async function runExcelFormulaToValueBinary(item,mode,selected){
  const XLSX=await ensureXlsx(),data=await item.file.arrayBuffer(),wb=XLSX.read(data,{type:'array',cellFormula:true,cellDates:true,cellNF:true,cellText:true,cellStyles:true,bookVBA:true});
  let converted=0,skipped=0,externalFound=0,totalFormula=0;
  for(let si=0;si<selected.length;si++){
    const name=selected[si],ws=wb.Sheets[name];if(!ws)continue;
    for(const addr of Object.keys(ws)){if(addr[0]==='!')continue;const cell=ws[addr];if(!cell?.f)continue;totalFormula++;const external=excelFormulaIsExternal(cell.f);if(external)externalFound++;if(mode!=='all'&&!external)continue;if(cell.v===undefined||cell.v===null){skipped++;continue}delete cell.f;delete cell.F;converted++}
    setStatus(`${mode==='external'?'Break Links':'Dán Value'}: ${name} · ${converted.toLocaleString('vi-VN')} ô`,Math.round((si+1)/selected.length*70));await sleep();
  }
  const meta=excelOutputMeta('xlsb'),arr=XLSX.write(wb,{type:'array',bookType:'xlsb',cellStyles:true,bookVBA:true,compression:true});
  const suffix=mode==='external'?'_break_links':'_values';await saveBlob(new Blob([arr],{type:meta.mime}),`${baseName(item.file.name)}${suffix}${meta.ext}`,meta.label);
  return {converted,skipped,externalFound,totalFormula,cleanupReason:'XLSB được ghi lại bằng SheetJS; không có bước sửa XML package như XLSX/XLSM.'};
}
async function runExcelFormulaToValue(mode){
  try{
    ...
    if(!item)throw new Error('Chưa chọn workbook.');
    const ext=fileExt(item.file.name);
    if(!['.xlsx','.xlsm','.xlsb'].includes(ext))throw new Error('Chức năng này hỗ trợ XLSX, XLSM và XLSB.');
    ...
    if(ext==='.xlsb'){
      const r=await runExcelFormulaToValueBinary(item,mode,selected);
      const baseReport=mode==='external'?`Đã Break Links ${r.converted.toLocaleString('vi-VN')}/${r.externalFound.toLocaleString('vi-VN')} công thức liên kết ngoài trong ${selected.length} sheet.`:`Đã dán Value ${r.converted.toLocaleString('vi-VN')}/${r.totalFormula.toLocaleString('vi-VN')} ô công thức trong ${selected.length} sheet.`;
      const skipped=r.skipped?` Có ${r.skipped.toLocaleString('vi-VN')} ô không có cached value nên được giữ nguyên.`:'';endBusy(baseReport+skipped+' '+r.cleanupReason);return;
    }
    const JSZip=await ensureZip(),zip=await JSZip.loadAsync(await item.file.arrayBuffer());
...
async function exportManagedWorkbook(){try{const item=state.excelFiles[0];if(!item)throw new Error('Chưa chọn workbook.');startBusy('Đang xuất workbook…');const meta=excelOutputMeta(excelSourceBookType(item));const arr=globalThis.XLSX.write(item.wb,{type:'array',bookType:meta.bookType,cellStyles:true,bookVBA:true,compression:true});await saveBlob(new Blob([arr],{type:meta.mime}),`${baseName(item.file.name)}_da_chinh_sua${meta.ext}`,meta.label);endBusy(`Đã xuất workbook ${meta.label} mới.`)}catch(e){showError(e)}}
...
async function runExcelMergeBooks(){try{startBusy('Đang gộp workbook…');const XLSX=globalThis.XLSX,out=XLSX.utils.book_new(),used=new Set();for(let i=0;i<state.excelFiles.length;i++){const item=state.excelFiles[i];for(const old of item.wb.SheetNames){let name=old.slice(0,31)||'Sheet',n=2;while(used.has(name))name=`${old.slice(0,27)}_${n++}`.slice(0,31);used.add(name);XLSX.utils.book_append_sheet(out,cloneWorksheet(item.wb.Sheets[old]),name)}setStatus(`Đang gộp file ${i+1}/${state.excelFiles.length}`,Math.round((i+1)/state.excelFiles.length*85))}const type=mainHost.querySelector('#excelMergeBooksFormat')?.value||'xlsx',meta=excelOutputMeta(type),arr=XLSX.write(out,{type:'array',bookType:meta.bookType,cellStyles:true,compression:true});await saveBlob(new Blob([arr],{type:meta.mime}),`Workbook_da_gop${meta.ext}`,meta.label);endBusy(`Đã gộp ${state.excelFiles.length} file.`);}catch(e){showError(e)}}
function renderExcelSplit(h){h.innerHTML=excelBrowseCard(`<div class="office-card"><h4>Tách mỗi sheet thành file riêng</h4><p class="office-card-note">Chọn XLSX/XLSB/XLSM để tách mỗi sheet thành XLSX, XLSB hoặc CSV; kết quả được đóng gói ZIP.</p><label class="office-field"><span>Định dạng</span><select id="excelSplitFormat"><option value="xlsx">Mỗi sheet thành XLSX</option><option value="xlsb">Mỗi sheet thành XLSB</option><option value="csv">Mỗi sheet thành CSV</option></select></label><div class="office-toolbar"><span class="spacer"></span><button class="office-btn primary" id="excelSplitRun" ${state.excelFiles.length?'':'disabled'}>Tách sheet</button></div></div>`);bindExcelLibrary();mainHost.querySelector('#excelSplitRun').onclick=runExcelSplit;}
async function runExcelSplit(){try{const item=state.excelFiles[0];if(!item)throw new Error('Chưa chọn workbook.');startBusy('Đang tách sheet…');const XLSX=globalThis.XLSX,fmt=mainHost.querySelector('#excelSplitFormat').value,outs=[];for(let i=0;i<item.wb.SheetNames.length;i++){const name=item.wb.SheetNames[i],ws=item.wb.Sheets[name];if(fmt==='csv')outs.push({name:`${safeName(name)}.csv`,blob:new Blob(['\ufeff'+XLSX.utils.sheet_to_csv(ws)],{type:'text/csv;charset=utf-8'})});else{const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,cloneWorksheet(ws),name.slice(0,31));const meta=excelOutputMeta(fmt==='xlsb'?'xlsb':'xlsx');outs.push({name:`${safeName(name)}${meta.ext}`,blob:new Blob([XLSX.write(wb,{type:'array',bookType:meta.bookType,cellStyles:true,compression:true})],{type:meta.mime})})}setStatus(`Đang tạo sheet ${i+1}/${item.wb.SheetNames.length}`,Math.round((i+1)/item.wb.SheetNames.length*80))}await downloadZip(outs,`${baseName(item.file.name)}_sheets.zip`);endBusy(`Đã tách ${outs.length} sheet.`);}catch(e){showError(e)}}
