...
function sanitizeLongComposerHtml(html){
  ...
  template.content.querySelectorAll('script,style,link,meta,base,iframe,object,embed,form,input,button,textarea,select,canvas,svg,video,audio').forEach(node=>node.remove());
  ...
  template.content.querySelectorAll('*').forEach(element=>{
    [...element.attributes].forEach(attribute=>{const name=attribute.name.toLowerCase(),value=attribute.value;if(name.startsWith('on')||name.includes(':')||name.startsWith('xmlns')||['id','class','srcdoc','contenteditable'].includes(name))element.removeAttribute(attribute.name);else if((name==='href'||name==='src')&&/^\s*javascript:/i.test(value))element.removeAttribute(attribute.name)});
    ...
    if(element.tagName==='IMG'){element.removeAttribute('srcset');element.removeAttribute('loading');element.removeAttribute('decoding');const src=element.getAttribute('src')||'';if(src&&!/^(data:image\/|blob:|https?:\/\/)/i.test(src))element.removeAttribute('src')}
...
async function inlineLongExportImages(root){const images=[...root.querySelectorAll('img')];for(let i=0;i<images.length;i++){assertNotAborted();const image=images[i],src=image.getAttribute('src')||'';if(!src)throw new Error(`Hình thứ ${i+1} chưa được nhúng. Hãy chèn lại bằng nút “Chèn ảnh”.`);if(!src.startsWith('data:image/')){try{const response=await fetch(src,{mode:'cors'});if(!response.ok)throw new Error(`HTTP ${response.status}`);image.src=await blobToDataUrl(await response.blob())}catch{throw new Error(`Hình thứ ${i+1} lấy từ nguồn ngoài không cho phép xuất. Hãy lưu hình về máy rồi chèn lại.`)}}await waitForLongImage(image,i+1)}}
...
async function runLongImageExport(editor,resultHost){
  ...
  try{
    ...
    const format=state.imageComposer.format,quality=state.imageComposer.quality/100,blob=await new Promise(resolve=>canvas.toBlob(resolve,format,quality));canvas.width=canvas.height=1;if(!blob)throw new Error('Trình duyệt không thể tạo ảnh ở kích thước này. Hãy giảm chiều rộng hoặc chia nội dung.');
