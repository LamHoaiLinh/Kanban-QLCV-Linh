...
function loadLongSvgImage(url){return new Promise((resolve,reject)=>{const image=new Image(),timer=setTimeout(()=>reject(new Error('Trình duyệt xử lý nội dung quá lâu.')),30000);image.onload=()=>{clearTimeout(timer);resolve(image)};image.onerror=()=>{clearTimeout(timer);reject(new Error('Không thể chuyển nội dung sang ảnh. Hãy thử bỏ định dạng phức tạp rồi xuất lại.'))};image.src=url})}

async function runLongImageExport(editor,resultHost){
  let measureHost=null,svgUrl='';
  try{
    ...
    const serialized=new XMLSerializer().serializeToString(root),svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${sourceWidth}" height="${sourceHeight}" viewBox="0 0 ${sourceWidth} ${sourceHeight}"><foreignObject width="100%" height="100%">${serialized}</foreignObject></svg>`,svgBlob=new Blob([svg],{type:'image/svg+xml;charset=utf-8'});svgUrl=URL.createObjectURL(svgBlob);const rendered=await loadLongSvgImage(svgUrl);assertNotAborted();
    const canvas=document.createElement('canvas');canvas.width=outputWidth;canvas.height=outputHeight;const context=canvas.getContext('2d',{alpha:false});context.fillStyle=state.imageComposer.bg;context.fillRect(0,0,outputWidth,outputHeight);context.drawImage(rendered,0,0,outputWidth,outputHeight);setStatus('Đang mã hóa tệp ảnh…',82);
  ...
  }catch(error){showError(error)}finally{measureHost?.remove();if(svgUrl)URL.revokeObjectURL(svgUrl)}
}
