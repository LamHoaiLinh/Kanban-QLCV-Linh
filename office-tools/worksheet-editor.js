...
class WorksheetEditor{
  ...
  template(){return `
    <div class="ws-top">
      ...
      <div class="ws-toolbar">
        ...
        <span class="ws-divider"></span><span class="ws-label">Màu</span><input class="ws-color" id="wsColor" type="color" value="${escapeHtml(this.color)}" title="Màu chữ / nét. Khi đang chọn chữ sẽ đổi đúng phần chữ được chọn.">
        <span class="ws-label">Nền chữ</span><input class="ws-color" id="wsTextBg" type="color" value="${escapeHtml(this.textBg)}" title="Tô nền phần chữ đang chọn"><button class="ws-btn ws-mini-btn" id="wsClearTextBg" title="Bỏ màu nền của phần chữ đang chọn">✕ nền</button>
        <span class="ws-label">Cỡ chữ</span><input class="ws-number" id="wsFontSize" type="number" min="10" max="100" value="${this.fontSize}" title="Chọn cả khung hoặc bôi đen một phần chữ rồi tăng/giảm cỡ">
  ...
  bindUi(){
    ...
    const preserveTextSelection=()=>this.captureEditingSelection();
    ['#wsColor','#wsTextBg','#wsFontSize','#wsClearTextBg'].forEach(sel=>q(sel)?.addEventListener('pointerdown',preserveTextSelection,true));
    q('#wsColor').oninput=e=>{this.color=e.target.value;localStorage.setItem('gk_ws_color',this.color);this.applyTextToolbarStyle('color',this.color)};
    q('#wsTextBg').oninput=e=>{this.textBg=e.target.value;localStorage.setItem('gk_ws_text_bg',this.textBg);this.applyTextToolbarStyle('background',this.textBg)};
    q('#wsClearTextBg').onclick=()=>{this.captureEditingSelection();this.applyTextToolbarStyle('background',null)};
    q('#wsFontSize').oninput=e=>{this.fontSize=clamp(Number(e.target.value)||28,10,100);e.target.value=this.fontSize;localStorage.setItem('gk_ws_font',this.fontSize);this.applyTextToolbarStyle('fontSize',this.fontSize)};
    q('#wsFontSize').onchange=q('#wsFontSize').oninput;
  ...
  finishCurrentTextEdit(){
    if(!this.editingTextKey)return;const [pageId,annId]=this.editingTextKey.split(':');const page=this.pages.find(p=>p.id===pageId);const ann=page&&findAnn(page,annId);const el=page&&this.pageDom.get(page.id)?.textLayer.querySelector(`[data-ann-id="${cssEscape(annId)}"]`);if(el&&ann)syncAnnFromEditor(el,ann);this.editingTextKey=null;this.savedTextSelection=null;if(el){el.contentEditable='false';el.classList.remove('editing')}if(page)this.renderTextLayer(page);this.markDirty();
  }
  captureEditingSelection(){
    if(!this.editingTextKey)return null;const [pageId,annId]=this.editingTextKey.split(':');const el=this.pageDom.get(pageId)?.textLayer.querySelector(`[data-ann-id="${cssEscape(annId)}"]`);const sel=el&&getSelectionOffsets(el);if(sel)this.savedTextSelection={pageId,annId,...sel};return this.savedTextSelection;
  }
  onSelectionChange(){
    if(!this.editingTextKey)return;const saved=this.captureEditingSelection();if(!saved)return;const page=this.pages.find(p=>p.id===saved.pageId);const ann=page&&findAnn(page,saved.annId);if(ann)this.syncToolbarFromText(ann,saved.start);
  }
  syncToolbarFromText(ann,offset=0){
    if(!this.root||!ann)return;const style=textStyleAt(ann,offset);const active=document.activeElement;if(active?.id!=='wsFontSize'){this.fontSize=clamp(Number(style.fontSize||ann.fontSize||this.fontSize),10,100);this.root.querySelector('#wsFontSize').value=this.fontSize}if(active?.id!=='wsColor'&&style.color){this.color=style.color;this.root.querySelector('#wsColor').value=toHexColor(style.color,this.color)}if(active?.id!=='wsTextBg'&&style.background){this.textBg=style.background;this.root.querySelector('#wsTextBg').value=toHexColor(style.background,this.textBg)}
  }
...
function drawTextAnnotation(ctx,a){
  ctx.save();ctx.textBaseline='top';const runs=ensureTextRuns(a);let x=a.x,y=a.y,lineH=Math.max(1,(a.fontSize||28)*1.18);
  for(const r of runs){const parts=String(r.text||'').split('\n');for(let i=0;i<parts.length;i++){const fs=Number(r.fontSize||a.fontSize||28);ctx.font=`${r.italic?'italic ':''}${r.bold?'700 ':'400 '}${fs}px "Segoe UI", Arial, sans-serif`;const seg=parts[i];const w=ctx.measureText(seg).width;const rh=fs*1.18;if(r.background&&seg){ctx.save();ctx.fillStyle=r.background;ctx.fillRect(x,y,w,rh);ctx.restore()}ctx.fillStyle=r.color||a.color||'#166fc0';if(seg)ctx.fillText(seg,x,y);x+=w;lineH=Math.max(lineH,rh);if(i<parts.length-1){x=a.x;y+=lineH;lineH=Math.max(1,(a.fontSize||28)*1.18)}}}ctx.restore()
}
function baseTextStyle(a){return {fontSize:Number(a.fontSize||28),color:a.color||'#166fc0',background:null,bold:Boolean(a.bold),italic:Boolean(a.italic)}}
function ensureTextRuns(a){
  const base=baseTextStyle(a);if(Array.isArray(a.runs)&&a.runs.length){a.runs=a.runs.map(r=>({text:String(r.text??''),fontSize:Number(r.fontSize||base.fontSize),color:r.color||base.color,background:r.background||null,bold:r.bold??base.bold,italic:r.italic??base.italic}));a.text=a.runs.map(r=>r.text).join('');return a.runs}
  const text=String(a.text||'');a.runs=text?[{text,...base}]:[];return a.runs
}
function textLength(a){return ensureTextRuns(a).reduce((n,r)=>n+r.text.length,0)}
function sameRunStyle(a,b){return Number(a.fontSize)===Number(b.fontSize)&&a.color===b.color&&(a.background||null)===(b.background||null)&&Boolean(a.bold)===Boolean(b.bold)&&Boolean(a.italic)===Boolean(b.italic)}
function mergeRuns(runs){const out=[];for(const r of runs){if(!r.text)continue;const n={...r,text:String(r.text)};const last=out[out.length-1];if(last&&sameRunStyle(last,n))last.text+=n.text;else out.push(n)}return out}
function applyRunStyle(a,start,end,prop,value){
  const runs=ensureTextRuns(a);const total=runs.reduce((n,r)=>n+r.text.length,0);const rawStart=Number(start)||0,rawEnd=Number(end)||0;start=clamp(Math.min(rawStart,rawEnd),0,total);end=clamp(Math.max(rawStart,rawEnd),0,total);if(start===end){start=0;end=total}let pos=0;const out=[];
  for(const r of runs){const rs=pos,re=pos+r.text.length;if(re<=start||rs>=end){out.push({...r});pos=re;continue}const l=Math.max(0,start-rs),rr=Math.min(r.text.length,end-rs);if(l>0)out.push({...r,text:r.text.slice(0,l)});const mid={...r,text:r.text.slice(l,rr)};if(prop==='fontSize')mid.fontSize=Number(value)||mid.fontSize;else if(prop==='color')mid.color=value||mid.color;else if(prop==='background')mid.background=value||null;out.push(mid);if(rr<r.text.length)out.push({...r,text:r.text.slice(rr)});pos=re}
  a.runs=mergeRuns(out);a.text=a.runs.map(r=>r.text).join('');return a.runs
}
function textStyleAt(a,offset=0){const runs=ensureTextRuns(a);if(!runs.length)return baseTextStyle(a);let pos=0;offset=Math.max(0,offset);for(const r of runs){if(offset<=pos+r.text.length)return r;pos+=r.text.length}return runs[runs.length-1]}
function renderRunsIntoElement(el,a){
  const runs=ensureTextRuns(a);el.replaceChildren();if(!runs.length)return;for(const r of runs){const span=document.createElement('span');span.textContent=r.text;span.style.fontSize=`${Number(r.fontSize||a.fontSize||28)}px`;span.style.color=r.color||a.color||'#166fc0';span.style.fontWeight=r.bold?'700':'400';span.style.fontStyle=r.italic?'italic':'normal';if(r.background)span.style.backgroundColor=r.background;el.appendChild(span)}
}
function syncAnnFromEditor(el,a){
  if(!el)return;const base=baseTextStyle(a),runs=[];const append=(text,style)=>{text=String(text||'').replace(/\u00a0/g,' ');if(!text)return;const r={text,fontSize:Number(style.fontSize||base.fontSize),color:style.color||base.color,background:style.background||null,bold:Boolean(style.bold),italic:Boolean(style.italic)};const last=runs[runs.length-1];if(last&&sameRunStyle(last,r))last.text+=text;else runs.push(r)};
  const walk=(node,ctx,isRoot=false)=>{if(node.nodeType===Node.TEXT_NODE){append(node.nodeValue,ctx);return}if(node.nodeType!==Node.ELEMENT_NODE)return;const tag=node.tagName;if(tag==='BR'){append('\n',ctx);return}const next={...ctx};const st=node.style||{};if(st.fontSize)next.fontSize=parseFloat(st.fontSize)||next.fontSize;if(st.color)next.color=toHexColor(st.color,next.color);if(st.backgroundColor&&st.backgroundColor!=='transparent'&&st.backgroundColor!=='rgba(0, 0, 0, 0)')next.background=toHexColor(st.backgroundColor,st.backgroundColor);if(st.backgroundColor==='transparent'||st.backgroundColor==='rgba(0, 0, 0, 0)')next.background=null;if(st.fontWeight)next.bold=st.fontWeight==='bold'||Number(st.fontWeight)>=600;if(st.fontStyle)next.italic=st.fontStyle==='italic';const block=!isRoot&&(tag==='DIV'||tag==='P');if(block&&runs.length&&!runs[runs.length-1].text.endsWith('\n'))append('\n',ctx);for(const ch of node.childNodes)walk(ch,next,false)};
  for(const ch of el.childNodes)walk(ch,base,false);a.runs=mergeRuns(runs);a.text=a.runs.map(r=>r.text).join('').replace(/\n+$/,'');if(a.text.length!==a.runs.map(r=>r.text).join('').length){let remain=a.text.length;const trim=[];for(const r of a.runs){if(remain<=0)break;const t=r.text.slice(0,remain);if(t)trim.push({...r,text:t});remain-=t.length}a.runs=trim}return a.runs
}
function getSelectionOffsets(el){const sel=getSelection();if(!sel||!sel.rangeCount)return null;const range=sel.getRangeAt(0);if(!el.contains(range.startContainer)||!el.contains(range.endContainer))return null;try{const pre=document.createRange();pre.selectNodeContents(el);pre.setEnd(range.startContainer,range.startOffset);const start=pre.toString().length;const pre2=document.createRange();pre2.selectNodeContents(el);pre2.setEnd(range.endContainer,range.endOffset);const end=pre2.toString().length;return {start:Math.min(start,end),end:Math.max(start,end)}}catch{return null}}
function setSelectionOffsets(el,start,end=start){const sel=getSelection();if(!sel)return;const texts=[];const tw=document.createTreeWalker(el,NodeFilter.SHOW_TEXT);let n,total=0;while(n=tw.nextNode()){texts.push({node:n,start:total,end:total+n.nodeValue.length});total+=n.nodeValue.length}start=clamp(start,0,total);end=clamp(end,0,total);const point=off=>{for(const t of texts){if(off<=t.end)return {node:t.node,offset:clamp(off-t.start,0,t.node.nodeValue.length)}}return texts.length?{node:texts[texts.length-1].node,offset:texts[texts.length-1].node.nodeValue.length}:{node:el,offset:0}};const a=point(start),b=point(end);const r=document.createRange();r.setStart(a.node,a.offset);r.setEnd(b.node,b.offset);sel.removeAllRanges();sel.addRange(r)}
function toHexColor(value,fallback='#166fc0'){if(!value)return fallback;if(/^#[0-9a-f]{6}$/i.test(value))return value;const m=String(value).match(/rgba?\(\s*(\d+)\D+(\d+)\D+(\d+)/i);if(!m)return fallback;return '#'+[m[1],m[2],m[3]].map(v=>clamp(Number(v),0,255).toString(16).padStart(2,'0')).join('')}
