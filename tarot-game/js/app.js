...
function createReading(cards){const count=cards.length,positions=SPREADS[count]||Array.from({length:count},(_,i)=>`Lá ${i+1}`),topic=refs.topic.value,question=refs.question.value.trim();const placed=cards.map((c,i)=>({...c,position:positions[i]}));const reading=interpret(placed,topic,question);currentReading={id:Date.now(),createdAt:new Date().toISOString(),topic,question,count,cards:placed.map(c=>({id:c.id,reversed:c.reversed,position:c.position})),summary:reading.summary,action:reading.action,conclusion:reading.conclusion};renderReading(currentReading);history.unshift(currentReading);history=history.slice(0,30);save(HISTORY_KEY,history);renderHistory()}
function interpret(cards,topic,question){const majors=cards.filter(c=>c.arcana==='major').length,upright=cards.filter(c=>!c.reversed).length,first=cards[0],last=cards[cards.length-1];const tone=upright>=Math.ceil(cards.length/2)?'Trải bài nghiêng về khả năng chủ động và mở ra hướng xử lý.':'Trải bài nhấn mạnh việc chậm lại, kiểm tra điểm mù và điều chỉnh trước khi tiến tiếp.';const majorText=majors?`${majors} lá Ẩn Chính cho thấy vấn đề này có ý nghĩa lớn hơn một tình huống nhất thời.`:'Các lá Ẩn Phụ cho thấy trọng tâm nằm ở lựa chọn và hành động thực tế hằng ngày.';const q=question?`Với câu hỏi “${question}”, `:'';const summary=`${q}${first.nameVi} mở ra chủ đề về ${first.keywords.slice(0,2).join(' và ')}. ${majorText} ${tone} Lá cuối ${last.nameVi} gợi ý hướng kết lại bằng việc ${lowerFirst(last.advice)}`;const actionItems=cards.slice(-Math.min(3,cards.length)).map(c=>c.advice.replace(/[.]$/,'')).filter((x,i,a)=>a.indexOf(x)===i);return{summary,action:`Gợi ý thực tế: ${actionItems.join(' • ')}.`,conclusion:buildConclusion(cards,topic,question)}}
function renderReading(reading){show(refs.result);const cards=reading.cards.map(saved=>{const base=TAROT_CARDS.find(c=>c.id===saved.id);return {...base,reversed:saved.reversed,position:saved.position}});refs.title.textContent=reading.question?`Chủ đề: ${TOPICS[reading.topic]}`:`Thông điệp ${TOPICS[reading.topic].toLowerCase()}`;reading.conclusion=reading.conclusion||buildConclusion(cards,reading.topic,reading.question||'');refs.conclusion.textContent=reading.conclusion;refs.summary.textContent=reading.summary;refs.action.textContent=reading.action;refs.cards.innerHTML=cards.map((card,index)=>`<article class="reading-card"><div class="card-image-wrap"><img class="card-image ${card.reversed?'reversed':''}" src="assets/cards/${card.img}" alt="${escAttr(card.nameVi)}" loading="lazy"></div><div class="card-copy"><span class="position">${esc(card.position)}</span><h3>${esc(card.nameVi)}</h3><div class="orientation">${card.reversed?'Lá ngược':'Lá xuôi'}</div><div class="keywords">${card.keywords.map(k=>`<span>${esc(k)}</span>`).join('')}</div><p>${esc(card.reversed?card.reversed:card.upright)}</p><p>${esc(topicInsight(card,reading.topic,index,cards.length))}</p><div class="advice"><strong>Lời gợi ý:</strong> ${esc(card.advice)}</div></div></article>`).join('')}
function buildConclusion(cards,topic,question){const upright=cards.filter(c=>!c.reversed).length,ratio=upright/cards.length,last=cards[cards.length-1],first=cards[0];let verdict=ratio>=.67?'Nghiêng về hướng có thể tiến tới':ratio<=.4?'Nghiêng về hướng chưa nên vội':'Có cơ hội, nhưng chỉ phù hợp khi đáp ứng đủ điều kiện';if(topic==='decision')verdict=ratio>=.67?'Nghiêng về CÓ, nhưng nên tiến từng bước':ratio<=.4?'Nghiêng về CHƯA, cần kiểm tra lại trước khi quyết định':'Chưa phải câu trả lời tuyệt đối; chỉ nên tiếp tục khi các điều kiện cốt lõi đã rõ';const focus={general:'ưu tiên điều quan trọng nhất và tránh phân tán',love:'đánh giá sự chân thành qua hành động, đồng thời giữ ranh giới rõ ràng',work:'làm rõ mục tiêu, trách nhiệm và tiêu chuẩn kết quả trước khi hành động',finance:'ưu tiên an toàn dòng tiền, số liệu và khả năng chịu rủi ro',family:'trao đổi thẳng nhưng mềm, tránh để kỳ vọng không nói ra tạo hiểu lầm',mind:'chậm lại để phân biệt cảm xúc nhất thời với nhu cầu thật sự',decision:'so sánh hệ quả ngắn hạn và dài hạn trước khi chốt'}[topic]||'hành động có kiểm chứng';const subject=question?`Đối với câu hỏi “${question}”`:`Đối với chủ đề ${TOPICS[topic].toLowerCase()}`;return `${subject}: ${verdict}. Trọng tâm là ${focus}; bước gần nhất nên làm là ${lowerFirst(last.advice)} Thông điệp này được củng cố bởi ${first.nameVi} và kết lại ở ${last.nameVi}.`}
